...
const PopoverContent = ({
  ...
  return (
    <PopoverPrimitive
      ...
      offset={offset}
      className={cx(
        "group/popover border-foreground/10 bg-overlay text-overlay-foreground max-w-xs min-w-(--trigger-width) origin-(--trigger-anchor-point) rounded-(--popover-radius) border shadow-xs outline-hidden transition-transform [--gutter:--spacing(2.5)] [--popover-radius:var(--radius-xl)] sm:text-sm dark:backdrop-saturate-200 **:[[role=dialog]]:[--gutter:--spacing(4)] [&:not(:has([data-slot=dialog-footer]))_[data-slot=dialog-body]]:pb-[var(--gutter)] [&:not(:has([data-slot=dialog-header]))_[data-slot=dialog-body]]:pt-[var(--gutter)]",
