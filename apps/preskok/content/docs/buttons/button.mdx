...
React Aria button props such as `onPress`, `type`, `isPending`, and `isDisabled` are also supported. Use the [Link](/typography/link) component with `buttonStyles` for links that should look like buttons.
