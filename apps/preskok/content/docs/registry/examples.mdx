...
### Custom style that extends @preskok-org/ui

The following registry item is a custom style that extends @preskok-org/ui. On `npx shadcn init`, it will:
...
The following registry item is a custom style that doesn't extend @preskok-org/ui. See the `extends: none` field.
...
- Add the `utils` registry item from the @preskok-org/ui registry.
...
The following style will init using @preskok-org/ui defaults and then add a custom `brand` color.
...
This blocks installs the `login-01` block from the @preskok-org/ui registry.
...
You can install a block fromt the @preskok-org/ui registry and override the primitives using your custom ones.

On `npx shadcn add`, the following will:

- Add the `login-01` block from the @preskok-org/ui registry.
