...
function makeTheme(input) {
  ...
  const lightAccent = isNeutralAccent
    ? "200"
    : determineShade(
        isNeutralAccent,
        isShade500Accent,
  ...
  const darkAccent = isNeutralAccent
    ? "800"
    : determineShade(
        isNeutralAccent,
        isShade500Accent,
  ...
  const lightAccentFg = isNeutralAccent
    ? "950"
    : determineForeground(isNeutralAccent, isShade400Accent, false)
  const darkAccentFg = isNeutralAccent
    ? "50"
    : determineForeground(isNeutralAccent, isShade400Accent, true)

  const chartShadesLight = isNeutralPrimary
    ? ["900", "700", "600", "500", "400"]
    : ["600", "400", "300", "200", "100"]
  const chartShadesDark = isNeutralPrimary
    ? ["800", "700", "500", "400", "300"]
    : ["700", "500", "400", "300", "200"]
...
async function scanVariables() {
  ...
  const variables = collection
    ? data.variables.filter((variable) => variable.variableCollectionId === collection.id)
    : []
...
async function setStoredState(state) {
  try {
    ...
    // Development plugins imported from a local manifest may not have a plugin
    // ID yet, and Figma blocks clientStorage in that state. The plugin still
    // works; it just won't remember the last selection between runs.
  }
}
...
figma.ui.onmessage = async (message) => {
  try {
    ...
    if (message.type === "save-mode") {
      ...
      figma.notify(
        result.created
          ? `Created ${result.modeName} theme mode.`
          : `Updated ${result.modeName} theme mode.`
      )
    ...
    if (message.type === "set-mode") {
      ...
      figma.notify(
        result.changed > 0
          ? "Applied variable mode to selection."
          : "Select one or more frames/components first."
      )
