...
const figmaInspectionRejectionCases: Array<{
  name: string
  createInput?: () => FigmaInspectionInput
  mutate: (input: FigmaInspectionInput) => void
  issueCode?: string
  noComponents?: boolean
}> = [
  {
    name: "rejects a visible instance without identity",
    createInput: createDashboardInspectionInput,
    mutate(input) {
      delete firstInspectionInstance(input).instance
    },
    issueCode: "figma_inspection_invalid",
  },
  {
    name: "ignores an instance beneath a hidden ancestor",
    mutate(input) {
      inspectionNode(input, "20:2").parentId = "20:hidden"
      input.inspection.nodes.splice(
        1,
        0,
  ...
  {
    name: "rejects an inspection root with a parent",
    mutate(input) {
      inspectionNode(input, input.inspection.rootNodeId).parentId =
        "outside-root"
    },
    issueCode: "figma_inspection_invalid",
  },
  {
    name: "rejects a disconnected node",
    mutate(input) {
      input.inspection.nodes.push({
        ...inspectionNode(input, "20:2"),
  ...
  },
  {
    ...
    mutate(input) {
      input.inspection.nodes.push(createInspectionNode({ nodeId: "20:2" }))
    },
    issueCode: "figma_inspection_invalid",
  },
  {
    name: "rejects identity attached to a non-instance",
    createInput: createDashboardInspectionInput,
    mutate(input) {
      firstInspectionInstance(input).type = "FRAME"
    },
    issueCode: "figma_inspection_invalid",
  },
  {
    name: "rejects descendants beneath an instance",
    mutate(input) {
      input.inspection.nodes.push(
        createInspectionNode({
          nodeId: "20:3",
  ...
  {
    name: "does not trust obsolete containment metadata",
    mutate(input) {
      const node = createInspectionNode({
        nodeId: "20:3",
        semanticFields: [
          { property: "fills", value: "#ff0000", tokenBound: false },
        ],
      })
      Object.assign(node, { insideInstance: true })
      input.inspection.nodes.push(node)
    },
    issueCode: "hardcoded_value",
  },
  {
    name: "rejects a visible subtree theme override",
    mutate(input) {
      inspectionNode(input, "20:2").explicitVariableModes = {
      ...
      if (!collection) {
        throw new Error("Expected a Mode fixture")
      }
      collection.modes.push({ modeId: "mode-dark", name: "Dark" })
    },
    issueCode: "theme_mode_mismatch",
  },
]
...
describe("Preskok Design MCP over stdio", () => {
  ...
  it.each(figmaInspectionRejectionCases)("$name", async (testCase) => {
    const input = (testCase.createInput ?? createCopiedButtonInspectionInput)()
...
function inspectionNode(input: FigmaInspectionInput, nodeId: string) {
  const node = input.inspection.nodes.find(
    (candidate) => candidate.nodeId === nodeId
  )
  if (!node) {
    throw new Error(`Expected inspection node ${nodeId}`)
  }
  return node
}

function firstInspectionInstance(input: FigmaInspectionInput) {
  const node = input.inspection.nodes.find(({ type }) => type === "INSTANCE")
  if (!node) {
    throw new Error("Expected an instance fixture")
  }
  return node
}

function createInspectionNode(
  overrides: Partial<FigmaInspectionNode>
): FigmaInspectionNode {
  return {
    nodeId: "20:fixture",
    parentId: "20:1",
